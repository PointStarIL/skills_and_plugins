...
## שער האימות: validate_docx.py

המנוע מספק גם את **הבדיקה** שהמסמך יצא תקין, לא רק את הבנייה. כל סקיל שבונה
DOCX מריץ את השער הזה לפני שהוא מציג את הקובץ למשתמש.

```bash
python3 "<...>/hebrew-docx-engine/scripts/validate_docx.py" out.docx
```

קוד יציאה: `0` תקין, `1` נמצאו הפרות, `2` שגיאת קלט. `--json` לפלט מכונה.
מקבל כמה קבצים בבת אחת.

שבעת הכללים שהוא אוכף, כולם נגזרים משלושת העקרונות הקריטיים שלמטה:

| כלל | מה נשבר אם זה מופר |
|---|---|
| `style` | פסקה בסגנון שאינו מסגנונות המנוע (למשל `List Paragraph`) |
| `run-rtl` | `run` בלי `<w:rtl/>`. Word יציג עברית כ-LTR ובפונט האנגלי |
| `run-rfonts` | `run` שקובע `rFonts`. שובר את גודל הטקסט האנגלי |
| `run-sz` | `run` שקובע `sz`. אותו נזק |
| `cell-jc` | פסקת תא שקובעת `w:jc`. מיישר **שמאלה** בפסקת bidi |
| `cell-bidi` | פסקת תא בלי `<w:bidi/>` |
| `table-bidivisual` | טבלה בלי `<w:bidiVisual/>`. העמודות יזרמו הפוך |
| `section-bidi` | `sectPr` בלי `<w:bidi/>`. הסקשן כולו LTR |
| `em-dash` | מקף ארוך או בינוני בטקסט |

**מה הוא לא בודק:** תוכן. הוא לא יודע אם הפסקה נכונה משפטית, אם התאריך אמיתי,
או אם הציטוט קיים במקור. הוא בודק שהמסמך ייפתח ב-Word כמסמך עברי תקין.

### ⚠️ פסקת גוף אינה נושאת `w:bidi`, וזה תקין

נמדד על פלט אמיתי של המנוע: `w:bidi` ברמת הפסקה מופיע **רק בפסקאות שבתוך תאי
טבלה**. פסקאות הגוף מקבלות את ה-RTL שלהן מהסגנון ומה-`sectPr`.
**אל תוסיף כלל שדורש `w:bidi` בכל פסקה** — הוא יכשיל כל מסמך תקין במערכת.
הבדיקה `test_body_paragraphs_need_no_bidi` נועלת את ההתנהגות הזו.

### בדיקות

`tests/test_validate_docx.py` מכיל מקרה מפורש לכל אחד מתשעת הכללים, שמפר אותו
בכוונה ודורש שהכלל **הנכון** ידווח. הסיבה: ולידטור שלא נכשל כשצריך גרוע מאין
ולידטור, כי הוא מייצר ראיה שקרית לכך שהמסמך נבדק.

```bash
python3 tests/test_validate_docx.py
```
