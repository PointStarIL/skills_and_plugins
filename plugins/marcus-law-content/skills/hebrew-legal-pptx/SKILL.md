---
name: hebrew-legal-pptx
description: יוצר, קורא ועורך מצגות פאורפוינט (.pptx) בעברית עם עיצוב מודרני־מאופק המתאים לקהל משפטי. גרסה v3, תומך גם ביצירת תמונות AI דרך kie.ai והוספתן למצגת (שקף תמונה מלא או טקסט-עם-תמונה-בצד). משתמש בפונטים Suez One (כותרות) + Rubik (גוף) מ-Google Fonts, מטמיע את הפונטים בתוך ה-PPTX, ומגדיר Complex Script typeface (cs) כדי ש-PowerPoint יציג את הפונטים נכון לעברית.
---

# מצגות עברית בסגנון מקצועי-משפטי (גרסה 3)
